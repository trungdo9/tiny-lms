...
If your host cannot reach Supabase's direct `db.<project-ref>.supabase.co:5432` endpoint (common on IPv4-only networks), Prisma may fail with `ENETUNREACH` or `P1001`. In that case, set `DATABASE_URL` to the Supabase pooler connection string instead of the direct database host.
...
## Testing and stabilization notes

### Shared regression assets

The current stabilization sprint relies on durable shared regression accounts plus a shared regression course/quiz fixture.

Current documentation status:
- testing/ops docs are updated to reflect the present stabilization state
- final sign-off is still pending backend connectivity verification and an authenticated regression rerun

Canonical source of truth:
- [`docs/testing/public-regression-checklist.md`](./docs/testing/public-regression-checklist.md)

That document currently covers:
- repeatable public and learner-facing regression workflow
- durable instructor/student/admin test identities
- shared regression course, lesson, and quiz fixture references
- known hotspots and current backend blockers

### Backend setup and recovery

For practical bring-up and environment recovery steps, see:
- [`docs/testing/backend-setup-and-recovery.md`](./docs/testing/backend-setup-and-recovery.md)
- [`backend/README.md`](./backend/README.md)

Important:
- backend schema sync currently uses `prisma db push`
- `backend/prisma/seed.ts` is a **destructive demo seed**, not a safe regression-fixture restore
- `backend/tmp-admin-login.js` is the shared admin recovery/bootstrap helper
...
- [Public Regression Checklist](./docs/testing/public-regression-checklist.md)
- [Backend Setup and Recovery](./docs/testing/backend-setup-and-recovery.md)
