...
Owner: QA / Huy_Tester
Status: Canonical regression checklist for public-facing and learner-facing routes
...
### Known product risk to verify every run
- [ ] Public header "Sign Up" button currently points to `/signup`, while the actual route is `/register`
  - Expected from product perspective: signup navigation should resolve to registration page
  - Current code suggests likely broken navigation or 404 risk
...
1. **Broken signup navigation**
   - Public header links to `/signup` while actual app route is `/register`
