...
async function main() {
    ...
    const htmlQuestionsWithOptions = await prisma.question.findMany({
        where: { id: { in: htmlQuestions.map((q) => q.id) } },
        include: { options: true },
    });
    const htmlQuestionMap = new Map(htmlQuestionsWithOptions.map((q) => [q.id, q]));
    ...
    for (let i = 0; i < htmlQuestions.length; i++) {
        ...
        const seededQuestion = htmlQuestionMap.get(htmlQuestions[i].id);
        const correctOptionIds = (seededQuestion?.options || [])
        ...
        await prisma.quizAnswer.create({
            data: {
                ...
                questionId: htmlQuestions[i].id,
                selectedOptions: i < 4 ? correctOptionIds : [],
                isCorrect: i < 4,
                scoreEarned: i < 4 ? 1 : 0,
            },
        });
