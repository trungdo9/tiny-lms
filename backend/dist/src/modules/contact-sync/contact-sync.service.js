...
const mailchimp_provider_1 = require("./providers/mailchimp.provider");
...
let ContactSyncService = ContactSyncService_1 = class ContactSyncService {
    ...
    logService;
    prisma;
    logger = new common_1.Logger(ContactSyncService_1.name);
    constructor(settingsService, logService, prisma) {
    ...
    async getProvider() {
        ...
        if (provider === 'mailchimp') {
            ...
            if (!apiKey?.value || !listId?.value)
                return null;
            this.logger.log('Using Mailchimp contact sync provider');
            return new mailchimp_provider_1.MailchimpProvider({
                apiKey: apiKey.value,
                listId: listId.value,
            });
        }
        if (provider === 'brevo') {
            ...
            return new brevo_provider_1.BrevoProvider({
                apiKey: apiKey.value,
    ...
    async syncUser(userId, trigger) {
        ...
        if (!contact)
            return null;
        const providerName = (await this.settingsService.get('contact_sync_provider'))?.value;
    ...
    async addUserTags(userId, tags, trigger) {
        const provider = await this.getProvider();
        if (!provider)
            return null;
        const user = await this.prisma.profile.findUnique({
    ...
    async removeUserTags(userId, tags, trigger) {
        const provider = await this.getProvider();
        if (!provider)
            return null;
        const user = await this.prisma.profile.findUnique({
    ...
    async addEnrollmentTag(userId, courseId, trigger) {
    ...
    async addCompletionTag(userId, courseId, trigger) {
        const course = await this.prisma.course.findUnique({
            where: { id: courseId },
            select: { slug: true },
        });
    ...
    async verifyConnection() {
        const provider = await this.getProvider();
        if (!provider)
            return { success: false, error: 'Contact sync is not enabled or configured' };
