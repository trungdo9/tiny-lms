...
export default function QuizIntroPage() {
  ...
  const returnTo = `/quizzes/${quizId}${searchParams?.toString() ? `?${searchParams.toString()}` : ''}`;
  ...
  const startQuiz = async () => {
    ...
    try {
      ...
      if (!session?.access_token) {
        ...
        setError('Please sign in to start this quiz.');
        setStarting(false);
        return;
      }
      ...
      if (message.toLowerCase().includes('not authenticated')) {
        setAuthRequired(true);
        setError('Please sign in to view and start this quiz.');
      } else {
        setError(message);
      }
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-2xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          {error && (
            <div className="mb-4 p-4 bg-red-50 text-red-600 rounded-lg">
              ...
              {authRequired && (
                ...
                >
                  Sign in and come back to this quiz
                </button>
              )}
            </div>
          )}
          ...
          <button
            onClick={authRequired ? goToLogin : startQuiz}
