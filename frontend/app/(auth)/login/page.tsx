...
function getSafeReturnTo(returnTo: string | null): string | null {
  ...
  if (returnTo.startsWith('//')) return null;
  if (returnTo.startsWith('/login') || returnTo.startsWith('/register')) return null;
  return returnTo;
}
...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(safeReturnTo || getRedirectPath(profile.role || 'student'));
    } catch {
...
export default function LoginPage() {
  return (
    <AuthProvider>
      <Suspense
        fallback={
          <div className="min-h-screen flex items-center justify-center bg-[#ffdb33]">
