...
export default function QuizzesPage() {
  ...
  const handleClone = (quiz: Quiz) => {
    if (!quiz.courseId) {
      setError('This quiz is not attached to a course yet, so clone must be done from a lesson.');
      return;
    }

    router.push(`/admin/courses/${quiz.courseId}/outline`);
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {quizzes.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <Link href="/admin/quizzes/create" className="text-blue-600 hover:underline">
                ...
                {quizzes.map((quiz) => (
                  <tr key={quiz.id} className="border-t">
                    <td className="px-4 py-3">
                      <Link href={`/admin/quizzes/${quiz.id}`} className="font-medium hover:text-blue-600">
                        {quiz.title}
                      </Link>
                      {quiz.description && (
